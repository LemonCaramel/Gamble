    ...
    public void refreshGUI() {
        ...
        for (String stock : manager.getStockMap().keySet()) {
            ...
            if (data.hasStock(stock)) {
                ...
                stack = InventoryUtil.createItemStack(Material.BOOK, header + "§f" + stock, " ", " §8-  §c현재 보유하고 있지 않는 주식입니다.", " §8-  §f현재가 §7: §6" + StockManager.getInstance().getStock(stock).getCurrent() + "§f원", " §8-  §f등락 §7: §f" + getPercent(StockManager.getInstance().getStock(stock).getPercent()), " §8-  §f우클릭시 1주 구매", " §8-  §f좌클릭시 1주 판매, 쉬프트 좌클릭시 전체 판매", " ");
            }
